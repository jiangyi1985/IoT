...
void Adafruit_ST7735::initR(uint8_t options) {
  ...
  if (options == INITR_GREENTAB) {
    ...
    _colstart = 24;
    _rowstart = 0;
  } else if (options == INITR_MINI160x80_GMT) {
    _height = ST7735_TFTWIDTH_80;
    _width = ST7735_TFTHEIGHT_160;
    displayInit(Rcmd2green160x80);
    _colstart = 26;//gmt's height(short side) first pixel start at idx 2
    _rowstart = 1;//gmt's width(long side) first pixel start at idx 1

    //gmt's display somehow inverted, swap the command here
    invertOnCommand = ST77XX_INVOFF;
    invertOffCommand = ST77XX_INVON;
    //and we need to call the method once, otherwise it starts in invert mode
    invertDisplay(false);
  } else {
  ...
  if (options == INITR_HALLOWING) {
    ...
    tabcolor = options;
    setRotation(3);//gmt's default upright position
  } else {
    tabcolor = options;
    setRotation(0);
  }
...
void Adafruit_ST7735::setRotation(uint8_t m) {
  ...
  case 0:
    ...
    if (tabcolor == INITR_144GREENTAB) {
      _height = ST7735_TFTHEIGHT_128;
      _width = ST7735_TFTWIDTH_128;
    } else if (tabcolor == INITR_MINI160x80 || tabcolor == INITR_MINI160x80_GMT) {
